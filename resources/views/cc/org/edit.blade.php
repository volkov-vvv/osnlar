...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div  class="col-xl-4">
                <form action="{{route('cc.org.update', $org->id)}}" method="post">
                    @csrf
                    @method('PATCH')
                    <div class="mb-3">
                        <label>Название организации</label>
                        <input name="email" type="email" class="form-control" value="{{$org->organization_title}}">
                        @error('email')
                    ...
                    <div class="mb-3">
                        <label>Email</label>
                        <input name="email" type="email" class="form-control" value="{{$org->email}}" disabled>
                        @error('email')
                        <div class="text-danger">{{$message}}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        <label>Фамилия</label>
                        <input name="lastname" type="text" class="form-control" aria-describedby="Название" value="{{$org->lastname}}" >
                        @error('lastname')
                    ...
                    <div class="mb-3">
                        <label>Имя</label>
                        <input name="firstname" type="text" class="form-control" aria-describedby="Название" value="{{$org->firstname}}" >
                        @error('firstname')
                        <div class="text-danger">Это поле необходимо для заполнения "{{$message}}"</div>
                        @enderror
                    </div>
                        ...
                        <select name="region_id" class="form-control select2">
                            @foreach($regions as $region)
                                <option value="{{$region->id}}"
                                    {{ $region->id == $org->region_id ? ' selected' : '' }}
                                >{{$region->title}}</option>
                            @endforeach
                        </select>
                        @error('region_id')
                        ...
                        <select name="responsible_id" class="form-control select2">
                            @foreach($users as $user)
                                <option value="{{$user->id}}"
                                    {{ $user->id == $org->responsible_id ? ' selected' : '' }}
                                >{{$user->name}}</option>
                            @endforeach
                        </select>
                        ...
                        <select name="status_id" class="form-control select2">
                            @foreach($statuses as $status)
                                <option value="{{$status->id}}"
                                    {{ $status->id == $org->status_id ? ' selected' : '' }}
                                >{{$status->title}}</option>
                            @endforeach
                        </select>
                        @error('status_id')
